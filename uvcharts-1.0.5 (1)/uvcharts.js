...
* @param {Object} config   Configuration of the graph being rendered
*/
uv.StepupWaterfallGraph = function (graphdef, config) {
  var self = this;
  uv.Graph.call(self, graphdef, config).setDefaults().init();
...
uv.StepupWaterfallGraph.prototype.drawHorizontalBars = function (idx, csum, tsum) {
  var self = this,
    len = self.categories.length,
    color = uv.util.getColorBand(self.config, idx),
    bargroup = self.bargroups[self.categories[idx]];
  ...
  bars.append('rect')
    ...
    .attr('width', 0)
    .attr('x', function (d, i) {
      var value = (d.value < 0) ? csum[i] + d.value : csum[i];
      csum[i] += d.value;
  ...
  bars.append('text')
    ...
    .transition()
      ...
      .delay(idx * self.config.effects.duration)
      .attr('x', function (d, i) {
        var value = d.value < 0 ? tsum[i] : tsum[i] + d.value;
        tsum[i] += d.value;
        return self.axes.hor.scale(value);
      });
...
uv.StepupWaterfallGraph.prototype.drawVerticalBars = function (idx, csum, tsum) {
  var self = this,
    color = uv.util.getColorBand(this.config, idx),
    len = self.categories.length;
  ...
  bars.append('text').attr('transform','scale(1,-1)')
      ...
      .transition()
        ...
        .attr('y', function (d, i) {
          tsum[i] += d.value;
          var value = d.value < 0 ? tsum[i] - d.value : tsum[i];
